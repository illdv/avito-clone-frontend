...
function* saveFavoriteSaga(favoritesAds: number[], selectedAdId: number, token: string) {
	...
	if (token) {
		try {
			yield call(FavoritesAPI.post, {favorites_ids: [selectedAdId]});
		} catch (e) {
...
function* removeFavoriteSaga(favoritesAds, selectedAdId, token, indexInFavorites) {
	...
	if (token) {
		try {
			yield call(FavoritesAPI.remove, {favorites_ids: [selectedAdId]});
		} catch (e) {
			yield call(errorHandler, e);
		}
	}
...
function* removeFavoriteAds(action: Action<{ favoritesId: number[] }>) {
	...
	if (token) {
		try {
			yield call(FavoritesAPI.remove, {favorites_ids: selectedFavoriteIds});
		} catch (e) {
			yield call(errorHandler, e);
		}
	}
...
function* get() {
	...
	try {
		const {data} = yield call(FavoritesAPI.get, {favorites_ids: favoritesId});
...
function* watcherUser() {
	yield [
		...
		takeEvery(favoritesActions.getFavoritesAds.REQUEST, get),
